...
set config=coalesce(config,'{}'::jsonb)||jsonb_build_object('legacy_action_type',action_type),
    action_type='report_preview', enabled=false, updated_at=now()
where action_type='report_template_export';
