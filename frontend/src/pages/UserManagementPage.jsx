...
export function UserManagementPage() {
  ...
  const isAdminOrOwner = profile && ["owner", "admin"].includes(profile.role);
  const canManage = isAdminOrOwner && hasPermission(permissions, "manage_users");
  const canCreate = isAdminOrOwner && (hasPermission(permissions, "create_users") || hasPermission(permissions, "invite_users"));
  const canManageAdmins = isAdminOrOwner && hasPermission(permissions, "manage_admins");
  const canEditProfile = isAdminOrOwner && hasPermission(permissions, "edit_user_profile");
  const canChangeRole = isAdminOrOwner && hasPermission(permissions, "change_user_role");
  const canDisable = isAdminOrOwner && hasPermission(permissions, "disable_users");
  const canResetPassword = isAdminOrOwner && hasPermission(permissions, "reset_user_password");
  const canViewAudit = isAdminOrOwner && hasPermission(permissions, "view_audit_logs");
    ...
    return (
      <section className="connection-error page-error" role="alert">
        <div>
          ...
          <h1>เฉพาะเจ้าของระบบและผู้ดูแลระบบเท่านั้น</h1>
          <p>หน้านี้จำกัดสิทธิ์เข้าถึงเฉพาะบัญชีผู้ใช้ที่มีบทบาทเป็น <strong>Owner (เจ้าของระบบ)</strong> หรือ <strong>Admin (ผู้ดูแลระบบ)</strong> เท่านั้น</p>
        </div>
      </section>
    );
  }
