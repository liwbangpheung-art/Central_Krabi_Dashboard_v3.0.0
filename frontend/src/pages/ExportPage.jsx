...
export function ExportPage() {
  const { config, api, profile, permissions, theme } = useOutletContext();
  ...
  const enabledSlides = useMemo(() => {
    return builderSlideOutline.filter((slide) => slide.enabled !== false);
  }, [builderSlideOutline]);

  useEffect(() => {
    if (previewSlideIndex === null) return;
    const handleKeyDown = (event) => {
      if (event.key === "ArrowRight" || event.key === "Right") {
        setPreviewSlideIndex((current) => Math.min(enabledSlides.length - 1, current + 1));
      } else if (event.key === "ArrowLeft" || event.key === "Left") {
        setPreviewSlideIndex((current) => Math.max(0, current - 1));
  ...
  }, [previewSlideIndex, enabledSlides]);
  ...
  return (
    <>
        ...
        <>
          ...
          <section className="report-builder-grid">
            ...
            <aside className="report-builder-summary">
              ...
              {builderState.report && (
                <button className="primary-button" type="button" style={{ background: "linear-gradient(135deg, #00897B, #004D40)", color: "white", marginBottom: "8px" }} onClick={() => setPreviewSlideIndex(0)}>
                  👁️ พรีวิวสไลด์นำเสนอเต็มจอ
                </button>
              )}
      ...
      {/* Slide Presentation Full-Screen Modal Preview */}
      {previewSlideIndex !== null && enabledSlides[previewSlideIndex] && (() => {
        const slide = enabledSlides[previewSlideIndex];
        const mData = slide.module ? builderState.report?.modules.find((m) => m.moduleInfo.id === slide.module) : null;
        
        const selectedTheme = reportThemeOptions.find((t) => t.id === builder.theme) || reportThemeOptions[0];
        const primaryColor = `#${selectedTheme.primary}`;
        const accentColor = `#${selectedTheme.accent}`;
        const surfaceColor = `#${selectedTheme.surface}`;
        ...
        return (
          <div className="slide-preview-overlay" role="dialog" aria-modal="true" aria-label="Virtual Slide Preview">
            <div className="slide-preview-header">
              <h3>🖥️ พรีวิวสไลด์นำเสนอเสมือนจริง ({previewSlideIndex + 1} / {enabledSlides.length})</h3>
              <div>
                <span style={{ marginRight: "16px", fontSize: "0.9rem", opacity: 0.8 }}>เลย์เอาต์: {slideLayoutOptions.find((l) => l.id === slide.layout)?.label || slide.layout || "Auto"}</span>
                <span style={{ fontSize: "0.9rem", opacity: 0.8 }}>ธีมสไลด์: {selectedTheme.label}</span>
              </div>
            </div>

            <div className="slide-canvas" style={slideStyle}>
              {/* Render Slide Content based on Type */}
              {slide.type === "cover" && (
                <div style={{ display: "flex", flexDirection: "column", justifyContent: "space-between", height: "100%" }}>
                  <div style={{ borderLeft: `6px solid ${accentColor}`, paddingLeft: "28px", margin: "8% 0 4% 0" }}>
                    <span style={{ fontSize: "1.1rem", fontWeight: "700", textTransform: "uppercase", color: accentColor, letterSpacing: "0.05em" }}>ENVIRONMENTAL REPORT</span>
                    <h1 style={{ fontSize: "2.8rem", margin: "14px 0", fontWeight: "900", lineHeight: "1.2", color: primaryColor }}>รายงานสรุปข้อมูลการจัดการของเสีย<br />และเศษวัสดุรีไซเคิล</h1>
                    <p style={{ fontSize: "1.3rem", opacity: 0.85, margin: "8px 0 0" }}>งวดรายงานประจำช่วง: {builderState.report?.periodLabel}</p>
                  </div>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-end", borderTop: `1px solid ${primaryColor}22`, paddingTop: "18px", fontSize: "0.95rem", opacity: 0.8 }}>
                    <span>จัดทำสำหรับ: <strong>{config.organizationName}</strong></span>
                    <span>พัฒนาโดย: ระบบการวิเคราะห์อัตโนมัติ (Central Krabi Dashboard)</span>
                  </div>
                </div>
              )}
              ...
              {slide.type === "summary" && (
                <div style={{ display: "flex", flexDirection: "column", justifyContent: "space-between", height: "100%" }}>
                  <div>
                    <h2>สรุปผู้บริหาร (Executive Summary)</h2>
                    <div style={{ display: "grid", gridTemplateColumns: "1fr 1.3fr", gap: "28px", marginTop: "12px" }}>
                      <div style={{ display: "flex", flexDirection: "column", gap: "12px" }}>
                        <h3 style={{ borderBottom: `1px solid ${accentColor}44`, paddingBottom: "6px" }}>📊 ยอดรวมรายหมวดข้อมูล</h3>
                        {builderState.report?.modules.map((m) => (
                          <div key={m.moduleInfo.id} style={{ background: `${primaryColor}06`, padding: "14px", borderRadius: "10px", border: `1px solid ${primaryColor}11` }}>
                            <strong style={{ fontSize: "0.95rem" }}>{m.moduleInfo.label}</strong>
                            <div style={{ fontSize: "1.6rem", fontWeight: "900", color: accentColor, marginTop: "4px" }}>
                              {formatNumber(m.data.kpis.grandTotal)} <small style={{ fontSize: "0.85rem", fontWeight: "500", color: primaryColor }}>{m.data.unit}</small>
                            </div>
                          </div>
                        ))}
                      </div>
                      <div>
                        <h3 style={{ borderBottom: `1px solid ${accentColor}44`, paddingBottom: "6px" }}>💡 ประเด็นสำคัญประจำงวด</h3>
                        <ul style={{ lineHeight: "1.55", fontSize: "0.95rem", paddingLeft: "16px", marginTop: "10px" }}>
                          {builderState.report?.summary?.insights.slice(0, 4).map((ins, i) => (
                            <li key={i} style={{ marginBottom: "8px" }}>{ins}</li>
                          ))}
                        </ul>
                      </div>
                    </div>
                  </div>
                  <div style={{ display: "flex", justifyContent: "space-between", fontSize: "0.85rem", opacity: 0.7, borderTop: `1px solid ${primaryColor}22`, paddingTop: "12px" }}>
                    <span>{config.organizationName} • {builderState.report?.periodLabel}</span>
                    <span>หน้า 2</span>
                  </div>
                </div>
              )}

              {slide.type === "data_quality" && (
                <div style={{ display: "flex", flexDirection: "column", justifyContent: "space-between", height: "100%" }}>
                  <div>
                    <h2>ความครบถ้วนและคุณภาพข้อมูล (Data Quality Audit)</h2>
                    <div style={{ display: "grid", gridTemplateColumns: "1fr 1.2fr", gap: "28px", marginTop: "16px" }}>
                      <div style={{ textAlign: "center", background: `${primaryColor}06`, padding: "24px", borderRadius: "16px", border: `1px solid ${primaryColor}11` }}>
                        <h3 style={{ margin: 0 }}>ความสมบูรณ์ของชุดข้อมูล</h3>
                        <div style={{ fontSize: "4.5rem", fontWeight: "900", color: accentColor, margin: "12px 0", lineHeight: 1 }}>
                          {formatNumber(builderState.report?.dataQuality?.summary?.completeness_percent, 1)}%
                        </div>
                        <p style={{ margin: 0, opacity: 0.8 }}>ข้อมูลสูญหาย {builderState.report?.dataQuality?.summary?.missing_cells} ช่องข้อมูล</p>
                      </div>
                      <div>
                        <h3 style={{ borderBottom: `1px solid ${accentColor}44`, paddingBottom: "6px" }}>🔒 สถานะงวดและข้อแนะนำ</h3>
                        <p style={{ margin: "8px 0 12px" }}>สถานะงวดข้อมูล: <strong>{builderState.report?.dataQuality?.period?.status_label || "เปิดให้แก้ไขข้อมูลได้"}</strong></p>
                        <ul style={{ lineHeight: "1.5", fontSize: "0.9rem", paddingLeft: "16px" }}>
                          {builderState.report?.dataQuality?.summary?.warnings?.slice(0, 3).map((w, i) => (
                  ...
                  <div style={{ display: "flex", justifyContent: "space-between", fontSize: "0.85rem", opacity: 0.7, borderTop: `1px solid ${primaryColor}22`, paddingTop: "12px" }}>
                    <span>{config.organizationName} • {builderState.report?.periodLabel}</span>
                    <span>หน้า {slide.no}</span>
                  </div>
                </div>
              )}

              {slide.type === "module_chart" && mData && (
                <div style={{ display: "flex", flexDirection: "column", justifyContent: "space-between", height: "100%" }}>
                  <div>
                    <h2>{mData.moduleInfo.label} — ดัชนีหลักและแนวโน้มสะสม</h2>
                    <div style={{ display: "grid", gridTemplateColumns: "1.8fr 1fr", gap: "24px", marginTop: "12px" }}>
                      <div style={{ background: "rgba(255, 255, 255, 0.7)", padding: "12px", borderRadius: "12px", border: "1px solid rgba(0,0,0,0.06)" }}>
                        <AnalyticsChart 
                          data={mData.data} 
                          rows={chartRows(mData.data)} 
                          chartType={builder.chartType} 
                          showLegend={true} 
                          showValues={true} 
                          includeTotal={false} 
                      ...
                      <div>
                        <h3 style={{ borderBottom: `1px solid ${accentColor}44`, paddingBottom: "6px" }}>🎯 สรุปผลตัวชี้วัด</h3>
                        <div style={{ background: `${primaryColor}06`, padding: "12px", borderRadius: "8px", margin: "8px 0" }}>
                          <span style={{ fontSize: "0.8rem", opacity: 0.7 }}>ปริมาณสะสมประจำงวด</span>
                          <div style={{ fontSize: "1.4rem", fontWeight: "900", color: accentColor }}>
                            {formatNumber(mData.data.kpis.grandTotal)} {mData.data.unit}
                          </div>
                        </div>
                        <p style={{ fontSize: "0.88rem", lineHeight: "1.45", margin: 0, opacity: 0.9 }}>
                          {mData.analysis.insights[0]}
                        </p>
                      </div>
                    </div>
                  </div>
              ...
              {slide.type === "category_breakdown" && mData && (
                <div style={{ display: "flex", flexDirection: "column", justifyContent: "space-between", height: "100%" }}>
                  <div>
                    <h2>{mData.moduleInfo.label} — สัดส่วนและอันดับขยะสูงสุด</h2>
                    <div style={{ display: "grid", gridTemplateColumns: "1.2fr 1fr", gap: "28px", marginTop: "16px" }}>
                      <div style={{ display: "flex", flexDirection: "column", gap: "8px" }}>
                        <h3 style={{ margin: 0 }}>📊 โครงสร้างข้อมูลแยกตามประเภทย่อย</h3>
                        {mData.data.categories.slice(0, 4).map((cat) => {
                          const pct = mData.data.kpis.grandTotal > 0 ? (cat.total / mData.data.kpis.grandTotal) * 100 : 0;
                          return (
                            <div key={cat.id}>
                              <div style={{ display: "flex", justifyContent: "space-between", marginBottom: "2px", fontSize: "0.88rem" }}>
                                <span>{cat.name_th}</span>
                                <strong>{formatNumber(cat.total)} {mData.data.unit} ({formatNumber(pct, 1)}%)</strong>
                              </div>
                              <div style={{ height: "6px", background: "rgba(0,0,0,0.06)", borderRadius: "3px", overflow: "hidden" }}>
                                <div style={{ width: `${pct}%`, height: "100%", background: cat.color_hex || accentColor }} />
                              </div>
                            </div>
                          );
                        })}
                      ...
                      <div>
                        <h3 style={{ borderBottom: `1px solid ${accentColor}44`, paddingBottom: "6px" }}>💡 บทวิเคราะห์สัดส่วน</h3>
                        <p style={{ fontSize: "0.88rem", lineHeight: "1.5", margin: 0, opacity: 0.9 }}>
                          {mData.analysis.insights[1] || "จากการวิเคราะห์โครงสร้างข้อมูลขยะและเศษวัสดุ พบว่าสัดส่วนส่วนใหญ่อยู่ในเกณฑ์มาตรฐานที่ยอมรับได้ และควรเพิ่มมาตรการการรณรงค์ตามแผนระยะยาว"}
                        </p>
                      </div>
                    </div>
                  </div>
              ...
              {slide.type === "data_table" && mData && (
                <div style={{ display: "flex", flexDirection: "column", justifyContent: "space-between", height: "100%" }}>
                  <div>
                    <h2>{mData.moduleInfo.label} — ตารางรายงานสรุปรายงวด</h2>
                    <div style={{ marginTop: "12px", overflowY: "auto", maxHeight: "250px", border: `1px solid ${primaryColor}11`, borderRadius: "8px" }}>
                      <table className="v3-data-table" style={{ width: "100%", fontSize: "0.85rem", borderCollapse: "collapse" }}>
                        <thead>
                          <tr style={{ background: `${primaryColor}08` }}>
                            <th style={{ padding: "8px" }}>รอบเวลา</th>
                            {mData.data.categories.map((c) => <th key={c.id} style={{ padding: "8px", textAlign: "right" }}>{c.name_th}</th>)}
                            <th style={{ padding: "8px", textAlign: "right" }}>รวม</th>
                          </tr>
                        </thead>
                        <tbody>
                          {mData.data.rows.slice(0, 6).map((row) => (
                            <tr key={row.period} style={{ borderBottom: `1px solid ${primaryColor}0c` }}>
                              <td style={{ padding: "8px" }}><strong>{row.label}</strong></td>
                              {mData.data.categories.map((c) => (
                                <td key={c.id} style={{ padding: "8px", textAlign: "right" }}>{formatNumber(row.values[c.code])}</td>
                              ))}
                              <td style={{ padding: "8px", textAlign: "right" }}><strong>{formatNumber(row.total)}</strong></td>
                            </tr>
                          ))}
                        </tbody>
                      </table>
              ...
              {slide.type === "recommendations" && (
                <div style={{ display: "flex", flexDirection: "column", justifyContent: "space-between", height: "100%" }}>
                  <div>
                    <h2>มาตรการและข้อเสนอแนะในการประหยัดทรัพยากร</h2>
                    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "28px", marginTop: "20px" }}>
                      <div style={{ background: `${primaryColor}06`, padding: "20px", borderRadius: "12px", border: `1px solid ${primaryColor}11` }}>
                        <h3 style={{ color: accentColor, fontWeight: "bold" }}>⚡ ระยะสั้น (Quick Wins)</h3>
                        <ul style={{ fontSize: "0.9rem", lineHeight: "1.55", paddingLeft: "16px", marginTop: "8px" }}>
                          <li style={{ marginBottom: "6px" }}>รณรงค์ลดขยะอินทรีย์ตามชั้นบริการและห้องครัว</li>
                          <li style={{ marginBottom: "6px" }}>เพิ่มจุดรับกล่องกระดาษและขวดแก้วในจุดทิ้งแยกประเภท</li>
                        </ul>
                      </div>
                      <div style={{ background: `${primaryColor}06`, padding: "20px", borderRadius: "12px", border: `1px solid ${primaryColor}11` }}>
                        <h3 style={{ color: accentColor, fontWeight: "bold" }}>🌳 ระยะยาว (Sustainability)</h3>
                        <ul style={{ fontSize: "0.9rem", lineHeight: "1.55", paddingLeft: "16px", marginTop: "8px" }}>
                          <li style={{ marginBottom: "6px" }}>ร่วมมือกับผู้ค้าขยะรีไซเคิลในการรายงานมูลค่าแยกรายละเอียด</li>
                          <li style={{ marginBottom: "6px" }}>ดำเนินการตามแนวทาง Zero Waste ตามนโยบายเครือ Central</li>
                        </ul>
                      </div>
                    </div>
                  </div>
            ...
            <div className="slide-preview-footer">
              <button className="slide-preview-close" type="button" onClick={() => setPreviewSlideIndex(null)}>
                ❌ ปิดหน้าต่างนำเสนอ (ESC)
              </button>
              <div className="slide-preview-controls">
                ...
                <span style={{ color: "white", fontSize: "1.1rem", fontWeight: "bold" }}>
                  สไลด์ที่ {previewSlideIndex + 1} จาก {enabledSlides.length}
                </span>
                <button
                  type="button"
                  disabled={previewSlideIndex === enabledSlides.length - 1}
                  onClick={() => setPreviewSlideIndex((p) => p + 1)}
                >
                  ถัดไป (Right) →
                </button>
