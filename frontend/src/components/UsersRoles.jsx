...
const emptyUser = { email: '', password: '', display_name: '', role: 'viewer', active: true }
      ...
      {tab === 'users' && (
        <div className="split-grid">
          <div className="card">
            ...
            <label className="field"><span>รหัสผ่านเริ่มต้น (อย่างน้อย 8 ตัว)</span><input type="password" minLength="8" value={userForm.password} onChange={e => setUserForm({ ...userForm, password: e.target.value })} /></label>
            ...
            <div className="form-actions bottom-actions"><button className="primary" onClick={() => saveUser.mutate(userForm)} disabled={saveUser.isPending}>Save User</button></div>
          </div>
                ...
                <tbody>
                  {(usersQuery.data || []).map(user => (
                    <tr key={user.id}>
                      ...
                      <td><button className="danger tiny" onClick={() => window.confirm('ลบผู้ใช้นี้หรือไม่?') && deleteUser.mutate(user.id)}>Delete</button></td>
                    </tr>
                  ))}
                </tbody>
              </table>
