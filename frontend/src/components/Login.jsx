...
export default function Login({ onLoginSuccess }) {
  ...
  async function resetPassword() {
    ...
    if (!authClient) return setError('ยังไม่ได้ตั้งค่า Supabase สำหรับ Frontend')
    const { error: resetError } = await authClient.auth.resetPasswordForEmail(email.trim(), { redirectTo: `${window.location.origin}/?set-password=1` })
    if (resetError) setError(readableLoginError(resetError)); else setResetMessage('ส่งลิงก์ตั้งรหัสผ่านแล้ว กรุณาตรวจอีเมล')
  }
  ...
  async function submit(event) {
    ...
    setSubmitting(true)
    try {
      if (!authClient) throw new Error('ยังไม่ได้ตั้งค่า Supabase สำหรับ Frontend')
      const { data, error: authError } = await authClient.auth.signInWithPassword({ email: email.trim(), password })
      if (authError) throw authError
      if (data?.session?.access_token) {
        localStorage.setItem('ckap_token', data.session.access_token)
        onLoginSuccess()
      } else {
