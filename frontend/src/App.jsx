...
import { CKAPV3PreviewPage } from "./pages/CKAPV3PreviewPage.jsx";


function ProtectedV3Preview({ config }) {
  const { session, loading } = useAuth();
  if (loading) return <LoadingScreen />;
  if (!session) return <Navigate to="/login" replace />;
  return <CKAPV3PreviewPage config={config} />;
}
    ...
    <AuthProvider supabase={supabase}>
      <BrowserRouter>
        <Routes>
          ...
          <Route path="/v3-preview" element={<ProtectedV3Preview config={config} />} />
