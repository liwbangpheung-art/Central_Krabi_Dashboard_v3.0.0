  ...
  router.get("/audit-logs", requirePermission("view_audit_logs"), async (req, res, next) => {
    try {
      ...
      let query = supabaseAdmin
        .from("audit_logs")
        .select("id,action,target_type,target_id,before_data,after_data,metadata,actor_user_id,created_at,profiles(id,full_name,email,role)", { count: "exact" })
        .order("created_at", { ascending: false })
        .range(offset, offset + limit - 1);

      if (actionFilter) query = query.ilike("action", `%${actionFilter}%`);
      if (userFilter) query = query.eq("actor_user_id", userFilter);
      ...
      res.json({
        items: (data ?? []).map((item) => ({
          ...
          performedBy: item.actor_user_id,
