...
export function createUsersRouter({ supabaseAdmin, authenticate }) {
  ...
  // Security layer: Only 'owner' and 'admin' can access user management endpoints
  router.use((req, res, next) => {
    if (!req.auth.profile || !["owner", "admin"].includes(req.auth.profile.role)) {
      return next(new HttpError(403, "ROLE_UNAUTHORIZED", "เฉพาะเจ้าของระบบ (owner) และแอดมิน (admin) เท่านั้นที่เข้าถึงระบบจัดการผู้ใช้นี้ได้"));
    }
    next();
  });
