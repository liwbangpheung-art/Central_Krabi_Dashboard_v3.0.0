...
async function resolveUser(req) {
  ...
  if (token) {
    try {
      ...
      if (supabase && userId) {
        const { data } = await supabase.from('profiles').select('*').eq('id', userId).eq('active', true).maybeSingle();
        if (data) {
          const hydrated = await hydrateUserPermissions(data);
          return { ...hydrated, profileExists: true };
        }
      }
      return { id: userId || null, email, display_name: 'Profile not found', role: 'blocked', permissions: [], profileExists: false };
    } catch (error) {
