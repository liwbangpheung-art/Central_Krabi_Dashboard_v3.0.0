...
app.post('/api/entries/batch', requireSupabase, requirePermission('entries.import'), async (req, res) => {
  ...
  if (!rows.length) return res.status(201).json({ inserted: 0, data: [] });
  const payload = [];
  for (const row of rows) {
...
app.get('/api/system-check', requireSupabase, requirePermission('settings.manage'), async (req, res) => {
  const requiredTables = ['profiles','roles','permissions','role_permissions','master_categories','data_entries','audit_logs','report_runs','automation_jobs'];
  ...
  res.json({
    ...
    backend: { ok: true, version: VERSION, node: process.version, environment: process.env.NODE_ENV || 'development' },
